...
val searchServicesOthers = mutableIntStateOf(0)
val searchServicesMaterialSupplier = mutableIntStateOf(0)
    ...
    if (openProviderSearchDialog.value) {
        ...
        providerResult.value.forEach { item ->
            if (item.name.uppercase().contains(searchName.value.uppercase().trim()) && searchName.value.trim().isNotEmpty())
                ...
                tmp.add(item)
            else if (item.servicesOthers==1 && searchServicesOthers.value==1)
                tmp.add(item)
            else if (item.servicesMaterialSupplier==1 && searchServicesMaterialSupplier.value==1)
                tmp.add(item)
